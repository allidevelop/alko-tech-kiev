...
- [x] 11.1 Функціонал "Обране" (Wishlist)
  - WishlistProvider — localStorage persistence (key: `alko_wishlist`)
  - Іконка сердечка на product cards (ProductActionsOverlay) та product detail (ProductActions)
  - Сторінка `/wishlist`: grid з thumbnail, назва, ціна, кнопка видалити
  - Лічильник в HeaderIcons з червоним fill при >0
  - Пустий стан: серце іконка + "Список бажань порожній" + "Перейти до каталогу"
  - API route `/api/products` для client-side fetch by IDs

- [x] 11.2 Функціонал "Порівняння" (Comparison)
  - ComparisonProvider — localStorage (key: `alko_comparison`), max 4 items
  - Іконка ваги на product cards та product detail page
  - Сторінка `/comparison`: таблиця — товари як колонки, характеристики як рядки
  - Динамічні spec_* рядки з metadata, ціна, бренд, артикул
  - Максимум 4 товари, counter (N/4) в заголовку
  - Лічильник на іконці в хедері (зелений акцент при >0)
  - Пустий стан: ваги іконка + "Список порівняння порожній" + кнопка каталог
  - Переклади UA/RU для обох функцій
